...
+function ($) {
    ...
    KobuVideo.prototype.initializeEvents = function () {
        var _ = this,
            src = _.$videoWrapper.find('source:last-child')[0];

        // Video events

        if (src) {
            src.addEventListener('error', _.listenerFunctions.error, false);
        } else {
            _.$video.addEventListener('error', _.listenerFunctions.error, false);
        }
    ...
    KobuVideo.prototype.clearEvents = function () {
        ...
        if (src) {
            src.removeEventListener('error', _.listenerFunctions.error, false);
        } else {
            _.$video.removeEventListener('error', _.listenerFunctions.error, false);
        }
