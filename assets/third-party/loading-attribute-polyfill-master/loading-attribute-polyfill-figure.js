...
function lazyLoadPolyfill(noscriptClass, rootMargin) {
	'use strict';
	...
	function prepareElement(noScriptTag) {
		...
		while (lazyArea.firstChild) {
			...
			if (
				...
				intersectionObserver.observe(imgSelector);
			}
	...
	function prepareElements() {
		...
		if ('objectFit' in document.documentElement.style === false) {
			document.querySelectorAll('.coverimg').forEach(function (image) {
...
lazyLoadPolyfill('loading-lazy', '256px 0px');